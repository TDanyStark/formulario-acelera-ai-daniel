<?php
/**
 * ACELERA override — LearnDash LD30 course navigation lesson row.
 *
 * Based on sfwd-lms/themes/ld30/templates/widgets/navigation/lesson-row.php
 * (LD 5.1.4, @version 4.21.4). Loaded only inside the ACELERA course via
 * Acelera_Template_Loader (filter `learndash_template`).
 *
 * Changes vs original:
 * - Welcome-gate (Fase 2) lock affordance rendered server-side: locked
 *   rows get aria-disabled="true" on the lesson anchor. The .acelera-locked
 *   row class itself keeps coming from the `learndash-nav-widget-lesson-class`
 *   filter (registered in Fase 2) which this template still applies; href
 *   neutralization stays JS-side (public JS).
 * - The internal section-heading block is kept intact but never fires in
 *   the plugin flow: the rows.php override renders headings itself and
 *   passes an empty 'sections' array (needed to wrap collapsible sections).
 *
 * @since 1.0.0
 *
...
/** This filter is documented in sfwd-lms/themes/ld30/templates/widgets/navigation/lesson-row.php */
$lesson_class = apply_filters( 'learndash-nav-widget-lesson-class', $lesson_class, $lesson );

// ACELERA (Fase 2/3): welcome-gate lock affordance, server-side.
$acelera_is_locked = class_exists( 'Acelera_Welcome_Gate' )
	&& Acelera_Welcome_Gate::is_lesson_locked( (int) $lesson['post']->ID, get_current_user_id() );
